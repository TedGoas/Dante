...
function* shadowDomIncludingInclusiveDescendantIterator(node) {
  yield node;

  if (node._shadowRoot) {
    yield* shadowDomIncludingInclusiveDescendantIterator(node._shadowRoot);
  }

  for (const child of domSymbolTree.childrenIterator(node)) {
    yield* shadowDomIncludingInclusiveDescendantIterator(child);
  }
}

// https://dom.spec.whatwg.org/#concept-shadow-including-descendant
function* shadowDomIncludingDescendantIterator(node) {
  if (node._shadowRoot) {
    yield* shadowDomIncludingInclusiveDescendantIterator(node._shadowRoot);
  }
